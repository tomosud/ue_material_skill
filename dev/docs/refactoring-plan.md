...
Status: repository/package restructuring completed; Phase 4 source audits continue; automated gates pass
...
- `skills/ue-material/catalog/nodes.json` contains 359 node entries.
...
- `skills/ue-material/catalog/functions.json` contains 82 Material Functions. Only 1 is marked verified, and 20 have
  `path_uncertain: true`.
...
- `skills/ue-material/SKILL.md` was mostly English but still contained Japanese operational text.
- `skills/ue-material/agents/openai.yaml` contained a Japanese `short_description`.
...
- Expand regression coverage for all Editor captures under `dev/fixtures/editor`, including duplicate Pin IDs across owners,
...
- Migrated source-audit rules to `skills/ue-material/references/source-verification.md`.
- Migrated source-checked Custom rules to `skills/ue-material/references/custom-expressions.md`.
- Migrated narrative Editor results to `skills/ue-material/catalog/editor-evidence.json`.
- Removed the completed root implementation plan and all completed `tasks/**/*.md` files.
- Replaced `tools/gen_manifest_tasks.py` with `dev/tools/gen_manifest.py`, which generates only the manifest and
...
- Added `skills/ue-material/catalog/node-evidence.json` with independent declaration, schema, description, and restriction
...
- Added `dev/tools/gen_node_evidence.py` and made `catalog_merge.py` require complete, valid evidence coverage.
...
- Moved unaudited legacy descriptions, notes, and nested field notes for 337 nodes to
  `skills/ue-material/catalog/legacy-node-prose.json`. The data is inactive search context and cannot be restored
...
- Added maintained, bounded audit fragments, now under `dev/catalog/audits/`; generated evidence validates
...
- Added `dev/tools/check_english.py` and a reviewed zero-debt baseline. Active `skills/ue-material/**` content contains zero
...
- Added `skills/ue-material/scripts/search_catalog.py` for deterministic search by class, free text, source path/symbol, Pin,
...
### Phase 8 repository and distribution split completed on 2026-07-14

- Moved the self-contained distributable payload to `skills/ue-material/` and established a one-way
  `dev -> skills/ue-material` dependency.
- Moved manifests, generated fragments, audits, fixtures, tests, maintenance tools, and development documents
  under `dev/`; removed the obsolete root extractor and tracked Python cache artifact.
- Consolidated the former `example/`, `examples/`, and `tests/fixtures/` trees under `dev/fixtures/` with
  descriptive `.t3d` and `.mgjson` names.
- Promoted the unreferenced root sample to the `substrate-simple-clear-coat` Editor fixture and added a focused
  regression proving four parsed nodes, three internal links, and intentional Material Root omission.
- Moved searchable legacy node wording into the distributable catalog so catalog search no longer depends on
  repository development data.
- Replaced local source-root paths in distribution metadata with `UE_SOURCE_ROOT` plus source-relative paths.
- Added the repository `README.md`, the existing source-refresh procedure, and a deterministic distribution
  boundary check.
- Regenerated the manifest from `C:\work\unreal\UnrealEngine-release`, revalidated all 359 declarations and
  six maintained audit fragments, and regenerated the distributable catalogs and compact index.
- All 26 offline tests pass. English lint, distribution validation, standalone search/validate/build/parse
  smoke tests, catalog regeneration, and `git diff --check` pass.
