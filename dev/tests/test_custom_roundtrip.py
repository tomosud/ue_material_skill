...
Run from the repository root:  python -m unittest discover -s dev/tests
No Editor required; dev/fixtures/editor/*.t3d are real UE 5.8 clipboard captures.
"""
...
SAMPLE_09 = (FIXTURE_ROOT / "editor" / "custom.t3d").read_text(encoding="utf-8-sig")
P01_COPYBACK = (FIXTURE_ROOT / "roundtrip" / "custom-lumasplit-copyback.t3d").read_text(
    encoding="utf-8-sig"
)
...
class ExistingSampleRegressionTests(unittest.TestCase):
    def test_all_samples_still_parse_clean(self):
        for sample in sorted((FIXTURE_ROOT / "editor").glob("*.t3d")):
    ...
    def test_substrate_root_selection_fixture(self):
        text = (FIXTURE_ROOT / "editor" / "substrate-simple-clear-coat.t3d").read_text(
            encoding="utf-8-sig"
        )
        document, stats = mgparse.convert_t3d(text, CATALOG)

        self.assertEqual(stats["nodes"], 4)
        self.assertEqual(stats["links"], 3)
        self.assertEqual(stats["unknown_classes"], 0)
        classes = {node["class"] for node in document["nodes"].values()}
        self.assertEqual(
            classes,
            {"SubstrateSimpleClearCoatBSDF", "Constant", "ScalarParameter"},
        )
        self.assertNotIn("MaterialGraphNode_Root", str(document))

    def test_sample02_duplicate_pinid_links(self):
        text = (FIXTURE_ROOT / "editor" / "math-chain.t3d").read_text(encoding="utf-8-sig")
        _, stats = mgparse.convert_t3d(text, CATALOG)
        self.assertEqual(stats["links"], 3)

    def test_sample04_texture_mask_output(self):
        text = (FIXTURE_ROOT / "editor" / "texture-mask.t3d").read_text(encoding="utf-8-sig")
