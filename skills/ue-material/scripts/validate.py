...
def load_catalog(path: Path | str | None = None) -> dict[str, Any]:
    target = Path(path) if path else skill_root() / "catalog" / "nodes.json"
...
def load_functions(path: Path | str | None = None) -> dict[str, Any]:
    target = Path(path) if path else skill_root() / "catalog" / "functions.json"
...
def load_node_evidence(path: Path | str | None = None) -> dict[str, Any]:
    target = Path(path) if path else skill_root() / "catalog" / "node-evidence.json"
    with target.open("r", encoding="utf-8-sig") as handle:
        value = json.load(handle, object_pairs_hook=_unique_object)
...
def load_editor_evidence(path: Path | str | None = None) -> dict[str, Any]:
    target = Path(path) if path else skill_root() / "catalog" / "editor-evidence.json"
    with target.open("r", encoding="utf-8-sig") as handle:
        value = json.load(handle, object_pairs_hook=_unique_object)
    if not isinstance(value, dict):
